...
const WardenLoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (uname === "") {
      swal("Error", "Enter a valid username", "error")
      return;
    } else if (pass === "") {
    ...
    await fetch("http://localhost:8800/warden/login", {
      ...
      .then(data => {
        ...
        if (result === 'SUCCESS' && token === 'wardenwarden') {
          ...
          swal("Please try after sometime", "Your registration is yet to be approved by the admin", "warning")
        } else {
        ...
        <div className="relative flex flex-col justify-center min-h-screen overflow-hidden">
          <div className="w-full p-6 m-auto bg-white rounded-md shadow-md lg:max-w-xl">
            ...
            <form className='mt-6'>
              ...
              <div>Haven't registered yet? <Link to='/warden-register' className='text-blue-800 underline'>Click here</Link> to register</div>
              <div className='mt-6'>
